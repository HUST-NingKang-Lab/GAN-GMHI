...
#setwd("D:/????ѩ/Project/GAN-GMHI/Combat-GMHI")
...
# I/O
microbiome_data_file <- "./corrected_metadata.csv"
fig3a_out <- "Fig3a.pdf"
fig3e_out <- "Fig3e.pdf"
fig3b_out <- "Fig3b.pdf"
fig3f_out <- "Fig3f.pdf"
...
overage_accuracy<-(sum(Healthy_GMHI$GMHI>0)+sum(Nonhealthy_GMHI$GMHI<0))*100/4346
...
dev.off()
compare_means(GMHI~Phenotype, data=GMHI_20)
...
my_comparsion <- list(c("ACVD","Healthy"),c("advanced adenoma","Healthy"),c("CRC","Healthy"),c("Crohns disease","Healthy"),c("IGT","Healthy"),c("Obesity","Healthy"),c("Overweight","Healthy"),c("Rheumatoid Arthritis","Healthy"),c("Symptomatic atherosclerosis","Healthy"),c("T2D","Healthy"),c("Ulcerative colitis","Healthy"),c("Underweight","Healthy"))
compare_means(GMHI~Phenotype_all,data=GMHI_phenotype_data)
pdf(fig3e_out)
fig3e <- ggplot(GMHI_phenotype_data, aes(x=Phenotype_all, y=GMHI, fill=Phenotype_all)) +
  geom_violin(trim=FALSE)+
  geom_boxplot(width=0.1,fill="white") + 
  stat_compare_means(comparisons = my_comparsion,label = "p.signif")+
