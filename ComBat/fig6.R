...
# I/O
Final_microbiome_data_4347_file <- "corrected_metadata.csv"
...
MH_species_file <- "./H_species.txt"
MN_species_file <- "./NH_species.txt"
...
overage_accuracy<-(sum(filter(fig6_alpha_GMHI_data_final,Phenotype=='Healthy')$GMHI>0)+sum(filter(fig6_alpha_GMHI_data_final,Phenotype=='Unhealthy')$GMHI<0))*100/679
report <- cbind(nrow(H_signature),nrow(NH_signature),Healthy_accuracy,Nonhealthy_accuracy,total_accuracy,overage_accuracy)
report
...
fig6_GMHI_boxplot <- ggboxplot(fig6_alpha_GMHI_data_final, x = "Phenotype_all1", y = "GMHI",
                               ...
                               add = "jitter",
                               short.panel.labs = FALSE,outlier.size=0,xlab = "", ylab = "Gut Microbiome Health Index (GMHI)",title = "Fig6b")+theme(axis.text.x = element_text(angle = 45, hjust = 0.5, vjust = 0.5))
