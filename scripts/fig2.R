...
colnames(alpha_shannon)[c(2,3,5)]<-c("Type",'Phenotype_all',"alpha")
...
pdf('fig3g_out.pdf')
fig3g<-ggplot(prevalence, aes(x=Type, y=prevalence, fill=Type)) +
  geom_violin(trim=FALSE)+geom_boxplot(width=0.1,fill="white")
...
fig3h <- ggplot(richness, aes(x=Type, y=richness, fill=Type)) +
  geom_violin(trim=FALSE)+geom_boxplot(width=0.1,fill="white")
