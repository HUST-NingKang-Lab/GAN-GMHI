...
test_final <- read.csv('test_GMHI.csv')
...
fig6b <-ggplot(test_final, aes(x=Phenotype, y=GMHI_new, fill=Phenotype)) + 
  geom_violin(trim=FALSE)+geom_boxplot(width=0.1, fill="white")+
  theme_bw()        
...
fig6c<-ggplot(test_final, aes(x=Phenotype_Batch, y=GMHI_new, fill=Phenotype_Batch)) +
  geom_violin(trim=FALSE)+geom_boxplot(width=0.1,fill="white")
