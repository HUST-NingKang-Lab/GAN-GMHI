...
validation_meta_file <- "validation_metadata1.csv"
study_wise_file <- "study_wise_data.txt"
MH_species_file <- "./H_species.csv"
MN_species_file <- "./NH_species.csv"
...
H <- read.csv(MH_species_file) # from Table 2
NH <- read.csv(MN_species_file) # from Table 2
H_species<-Healthy_associated_species#as.character( H$x)
NH_species<-Nonhealthy_associated_species#as.character( NH$x)

# Processing of meta-data from validation cohort
fig6<-data.frame((fig6[,c(3:544)]))
...
fig6_3<-sweep(fig6_1,2,colSums(fig6_1),`/`) # re-normalization to get relative abundances after removing unclassified and viruses
fig6_4<-(fig6_3)*100
...
Healthy_accuracy <- sum(filter(fig6_alpha_GMHI_data_final,Phenotype=='Healthy')$GMHI> 0)*100/118
Nonhealthy_accuracy <- sum(filter(fig6_alpha_GMHI_data_final,Phenotype=='Unhealthy')$GMHI< 0)*100/561
...
H_Un_count <- data.frame(table(fig6_alpha_GMHI_data1$Phenotype))


# phenotype_wise_validation_sample_counts
All_phenotype_counts <- data.frame(table(fig6_alpha_GMHI_data1$Phenotype_all)) 
fig6_alpha_GMHI_data1$Phenotype <- gsub(x = fig6_alpha_GMHI_data1$Phenotype, 
...
dev.off()
cliffDelta(data =fig6_alpha_GMHI_data1,GMHI~Phenotype)
...
pdf('fig6b_out1.pdf',height = 9,width = 16)
fig6_GMHI_boxplot <- ggboxplot(fig6_alpha_GMHI_data_final, x = "Phenotype_all1", y = "GMHI",
                               color = "Phenotype_all1",
                               add = "jitter",
                               short.panel.labs = FALSE,outlier.size=0,xlab = "", ylab = "Gut Microbiome Health Index (GMHI)",title = "Fig6b")
fig6_GMHI_boxplot +rremove("legend")+theme(axis.text=element_text(size=14,face="bold"),axis.title=element_text(size=14,face="bold"))+scale_color_manual(values=distinctColorPalette(21))
...
# End
Healthy_accuracy <- sum(filter(test_final,Phenotype=='Healthy')$G_GMHI> -0.393)*100/118
Nonhealthy_accuracy <- sum(filter(test_final,Phenotype=='Unhealthy')$G_GMHI< -0.393)*100/561
